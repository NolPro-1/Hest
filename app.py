...
def login():
    ...
    if not identity or not password:
        return jsonify({"error": "Введите никнейм/почту и пароль."}), 400

    users = load_users()
    user = None
    # Try to find by nickname or email
    for u in users:
        if u["nickname"].lower() == identity.lower() or u["email"].lower() == identity.lower():
            user = u
            break
    ...
    return jsonify(
        {
            ...
            "inventory": user.get("inventory", {"skins": [], "items": [], "achievements": []}),
        }
    )
...
def change_nickname():
    ...
    nickname = str(payload.get("nickname", "")).strip()
    password = str(payload.get("password", "")).strip()
    new_nickname = str(payload.get("new_nickname", "")).strip()

    if not nickname or not password or not new_nickname:
        return jsonify({"error": "Заполните все поля."}), 400
    if len(new_nickname) < 3:
        return jsonify({"error": "Никнейм минимум 3 символа."}), 400
    ...
    if not verify_password(user["password"], password):
        return jsonify({"error": "Неверный пароль."}), 400
    if any(u["nickname"].lower() == new_nickname.lower() and u["nickname"] != nickname for u in users):
        return jsonify({"error": "Этот никнейм уже занят."}), 400

    user["nickname"] = new_nickname
    save_users(users)
    return jsonify({"success": True, "new_nickname": new_nickname})
def login():
    payload = request.get_json(silent=True) or {}
